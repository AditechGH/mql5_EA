...
input bool   inputStopLossTrailing = false;                 // trailing stop loss?
...
void UpdateStopLoss () {
    ...
    // return if stop loss or fixed stop loss
    if(inputStopLossTrailing == 0 || !inputStopLossTrailing) return;
    ...
    for(int i = total - 1; i >= 0; i--) {
        ...
        if(magic == inputMagicNumber) {
            ...
            // calculate stop loss
            double currentPrice = type == POSITION_TYPE_BUY ? lastTick.bid : lastTick.ask; 
            int n               = type == POSITION_TYPE_BUY ? 1 : -1;
            double newSL        = NormalizeDouble(currentPrice - ((range.high - range.low) * inputStopLoss * 0.01 * n), _Digits);   
...
double CalculateLotSize(ENUM_ORDER_TYPE type, double ask, double pipPoints) {
    ...
    CheckLot(lotSize, type, ask);
 
    return NormalizeDouble(lotSize, 2);
}

double CheckLot(double &lotSize, ENUM_ORDER_TYPE type, double ask) {
    lotSize = 0;
